...
import com.csp.sample.customview.BuildConfig;
...
public class LogCat {
     ...
     * @param throwable 异常
     */
    public static void printLog(int level, String tag, String message, Throwable throwable) {
        if (message == null) {
            message = "";
        }
        String throwableStr = getStackTraceString(throwable);
        String newline = message.isEmpty() || throwableStr.isEmpty() ? "" : "\n";
        String[] messages = divideMessages(message + newline + throwableStr);
    ...
    public static void printStackTraceForDebug(int level, String explain, Throwable throwable) {
        ...
        printLog(level, tag, log, throwable);
    }
